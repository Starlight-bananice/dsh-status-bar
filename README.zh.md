...
底栏替换内置统计行，实时呈现会话状态（状态 · 模型 · 轮次 · 上下文 · 缓存 · TPS · 会话时长 · 任务 · 队列 · 错误），通过专属设置页统一管理：

| 逐段配置（设置页） | 随时开关与排序（信息段列表） |
|---|---|
| ![设置页](assets/screenshot-settings-page.png) | ![信息段列表](assets/screenshot-settings-segments.png) |

费用估算基于逐模型价格手册（支持峰谷计价），「用量与消耗」弹窗拆解每个会话的花费：

| 模型价格手册（逐模型单价 + 峰谷时段） | 用量与消耗弹窗（趋势图 · 统计卡 · 历史） |
|---|---|
| ![模型价格手册](assets/screenshot-price-book.png) | ![用量与消耗弹窗](assets/screenshot-usage-cost-dialog.png) |
