...
function estimateContentTokens(blocks: readonly ContentBlock[], depth = 0): number {
  let tokens = 0
  ...
  apply: (state, event) => {
    ...
    switch (event.type) {
      case 'assistant/chunk': {
        ...
        if (!fresh && state.exact) return state
        const book = {
          blocks: [...state.blocks],
        ...
        return {
          ...
          outputTokens,
          estimated: true,
          tokensPerSecond: ewmaRate(
            state.tokensPerSecond,
            instantRateOf(added, fresh ? null : state.prevOutputTime, event.time),
          ),
        }
      }
